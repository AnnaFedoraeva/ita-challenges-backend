...
interface CustomConverterTeam {
    //extends StarterChallengeConverterTeam, FromChallengeDocConverterTeam{
...
class DemoService {
    ...
    public void demo(){
        ChallengeDummy challenge = null; //obtenido de donde sea

        ChallengeDtoDummy challengeDto = converter.from(challenge)
                .toChallengeDtoWithOnlyBasicInfo(0.3f,1);
    }
}
...
Y porque este diseño?
No es más fácil/simple una classe con un(os) método(s) rollo:
    ChallengeDto convertToOnlyBasicInfo(Challenge challenge, 0.3, 1) ???
...
Hay varias arquitecturas en que algunas vistas (los dtos) son dependientes del modelo que
presentan (los documents).
...
parámetros el método ya tira -> no necesita trabajar con ningún atributo.
...
quieres usar, pero no importa en que clase ponerlos. Lo que te importaría de la clase son
...
atributos ya se arreglará cuando llegue el momento).
Así que necesitará N sets, o N constructores con visibilidad public.
Hipoteticamente sería posible que, des de un service, se hiciera set(SolutionDto)
...
Lo ideal sería que si un objeto de la clase X quiere lanzar un mensaje Y, solo tubiera acceso
a los que necesita + puede invocarlos sin mideo ni problemas.
...
para arreglar eso
3) Seguramente habría codigo duplicado -> habría que montar alguna estructura de herencia para
arreglarlo -> más clases -> más complejo
...
Tengo un cocinero que hoy puede cocinar un montón de platos.
Pero yo, como dueño del restaurante, eligo que primeros se pueden pedir hoy.
Y, si quisiera, puedo ofrecer un menú de primeros distinto a cada cliente.
Y, según que primero pida, le ofrezco un ménu de segundos u otro (e incluso podría variar según el cliente).
